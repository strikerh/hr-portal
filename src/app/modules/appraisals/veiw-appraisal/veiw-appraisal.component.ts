...
export class VeiwAppraisalComponent implements OnChanges, OnInit {
    ...
    constructor(
        private view: ViewAppraisalService,
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes['data'] && changes['data'].currentValue) {
            ...
            this.userFeedback = this.appraisal.employee_feedback;
            this.managerFeedback = this.appraisal.manager_feedback;
    ...
    parseResponseToJson(htmlResponse: string) {
        ...
        workSection.forEach((h3) => {
            ...
            const description = h3.nextElementSibling
                ? h3.nextElementSibling.querySelector('em')?.textContent.trim()
                : '';
